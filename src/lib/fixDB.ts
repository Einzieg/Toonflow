...
function extractRecoveredProjectName(scriptName?: string | null, chapterName?: string | null) {
  const source = scriptName?.trim() || chapterName?.trim() || "";
  if (!source) return "";

  const normalized = source.replace(/^#\s*/, "").trim();
  const epMatch = normalized.match(/^《?(.+?)》?\s*EP\d+/i);
  if (epMatch?.[1]) return epMatch[1].trim();

  return normalized.split(/[：:]/)[0].trim();
}

async function recoverMissingProjects(knex: Knex) {
  const projectSources = ["o_assets", "o_script", "o_novel", "o_outline", "o_storyboard", "o_video"] as const;
  const existingProjects = await db("o_project").select("id").orderBy("id", "desc");
  const fallbackProject = await db("o_project").orderBy("id", "desc").first();
  const existingIds = new Set(existingProjects.map((row) => row.id).filter(Boolean));
  const orphanIds = new Set<number>();

  for (const table of projectSources) {
    if (!(await knex.schema.hasTable(table))) continue;
    const rows = await db(table).whereNotNull("projectId").select("projectId");
    rows.forEach((row: any) => {
      const projectId = Number(row.projectId);
      if (Number.isFinite(projectId) && !existingIds.has(projectId)) {
        orphanIds.add(projectId);
      }
    });
  }

  for (const projectId of orphanIds) {
    const firstScript = await db("o_script").where("projectId", projectId).select("name").orderBy("id").first();
    const firstNovel = await db("o_novel").where("projectId", projectId).select("chapter").orderBy("id").first();
    const recoveredName = extractRecoveredProjectName(firstScript?.name, firstNovel?.chapter) || `恢复项目 ${projectId}`;

    await db("o_project").insert({
      id: projectId,
      projectType: fallbackProject?.projectType ?? "novel",
      imageModel: fallbackProject?.imageModel ?? "openai:google/gemini-3.1-flash-image-preview",
      imageQuality: fallbackProject?.imageQuality ?? "2K",
      videoModel: fallbackProject?.videoModel ?? "volcengine:doubao-seedance-2-0-260128",
      name: recoveredName,
      intro: fallbackProject?.intro ?? "",
      type: fallbackProject?.type ?? "",
      artStyle: fallbackProject?.artStyle ?? "realpeople_urban_modern",
      directorManual: fallbackProject?.directorManual ?? "Scifi_future_epic",
      mode: fallbackProject?.mode ?? '["textReference","videoReference","imageReference","audioReference"]',
      videoRatio: fallbackProject?.videoRatio ?? "16:9",
      createTime: fallbackProject?.createTime ?? projectId,
      userId: fallbackProject?.userId ?? 1,
    });

    console.log(`[fixDB] 恢复缺失项目: ${projectId} -> ${recoveredName}`);
  }
}
...
export default async (knex: Knex): Promise<void> => {
  ...
  const videoPromptGeneration = await db("o_prompt").where("type", "videoPromptGeneration").first();
  const videoPromptPatch: { data?: string; useData?: string } = {};
  if (videoPromptGeneration?.data != null) {
    videoPromptPatch.data = patchVideoPromptGenerationSeedance2Section(videoPromptGeneration.data);
  }
  if (videoPromptGeneration?.useData != null) {
    videoPromptPatch.useData = patchVideoPromptGenerationSeedance2Section(videoPromptGeneration.useData);
  }
  if (Object.keys(videoPromptPatch).length > 0) {
    await db("o_prompt").where("type", "videoPromptGeneration").update(videoPromptPatch);
  }
