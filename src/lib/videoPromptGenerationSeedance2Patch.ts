const seedance2SectionPattern = /### 三、Seedance 2\.0[\s\S]*?(?=### 四、Wan 2\.6)/;
const videoPromptDocPresetStart = "<!-- TOONFLOW:DOC_VIDEO_PROMPT_PRESETS:START -->";
const videoPromptDocPresetEnd = "<!-- TOONFLOW:DOC_VIDEO_PROMPT_PRESETS:END -->";

const videoPromptDocPresetSection = `## 文档增强：通用镜头预设与通用推理

来源：docs/通用 镜头预设.md、docs/通用 预设.md、docs/通用推理.md。以下规则适用于所有视频提示词模式。

- 参考图必须使用当前系统分配的 @图N；@图N 依据资产和分镜图的真实输入顺序绑定，不能按编号臆断角色/场景/道具类型。
- 文档中的 **@预设名** 只作为资产名映射规则理解；最终输出仍按当前模式要求使用 @图N、参考定义或纯文本，不输出 @角色库、@场景库、@物品库 或自造别名。
- 多参考图镜头必须逐一绑定角色、场景、道具和分镜图；不要只引用第一张图，也不要让一个 @图N 代替多个资产。
- 每个分镜都要从静态画面推理出视频动作轴：主动作、二级动作、微表情、身体姿态、焦点变化、镜头路径和情绪落点。
- 单镜头内部按完整时长组织起幅状态、中段推进、收束落点；时间轴要覆盖 duration，不能整段只写一张静态图。
- 镜头运动优先使用可执行类型：静止后轻推、推进/拉远、横移跟拍、升降、环绕、焦点转移、景别变化、手持轻晃、情绪化缓推。
- 声音、环境、光影只能基于 videoDesc 已有信息或逻辑必需信息补充，禁止新增未出现角色、道具、场景或剧情结果；当无台词或台词少于每秒约 2 字时，允许按剧情补充中文普通话对白、内心OS或画外音到每秒约 2-3 字。
- 每个最终视频提示词都必须包含 BGM/Background music/背景音乐 描述：根据 videoDesc 的剧情、情绪、动作强度和台词语气选择配乐气质、节奏、音量和起伏；BGM 必须低于对白，不使用歌词，不遮盖环境音/关键音效。
`;

export const seedance2PromptSection = `### 三、Seedance 2.0

#### 核心原则
- **保持当前系统输出结构**：继续使用 \`画面风格和类型\`、\`参考定义\`、\`生成一个由以下 N 个分镜组成的视频\`、\`分镜N {N}s:\`，不要改成单行序号格式
- **按轨道实际时长生成**：Seedance 2.0/2.0-Fast 使用当前轨道累计 duration 作为视频时长，不再固定补足到 15 秒；提示词中的所有 \`分镜N {N}s\` 时长相加必须严格等于 XML duration 总和，且不超过 15s
- **真人参考使用官方虚拟人像**：当角色资产配置了火山官方/授权素材 \`asset://...\` 时，该素材会作为接口参考图提交；提示词仍只按系统分配的 \`@图N\`/角色名描述，不输出 \`asset://\` 或真实素材 ID
- **衍生角色继承虚拟人像**：角色的服装、状态、表情等衍生资产只作为文字造型约束和工作流预览；进入 Seedance 2.x 视频生成时仍使用父角色的官方虚拟人像参考，不把衍生角色图片当真人参考图上传
- **分镜图只作构图辅助**：当分镜关联了官方虚拟人像角色时，视频接口优先接收角色 \`asset://\` 参考，分镜成图不再作为人物参考图上传；提示词需要用分镜 \`videoDesc\` 补足构图、动作和镜头信息
- **duration 字段优先**：当 XML \`duration\` 与 \`videoDesc\` 内部时长不一致时，以 XML \`duration\` 为准；\`videoDesc\` 内部时长只作为原始节奏参考
- **videoDesc 为主，prompt 为辅**：优先依据 \`videoDesc\` 生成运动镜头；\`prompt\` 只补充静态构图、景别、视角、主体站位和已明确道具关系，若冲突以 \`videoDesc\` 为准
- **静图转动态**：每个分镜都必须写成可直接生成视频的动态描述，不能只复述一张静态图
- **单镜头内要有动作递进**：优先组织成“起幅状态 → 中段推进 → 收束落点”，自然使用“先/随后/最后”等词，让动作、神态和镜头在单个分镜内连续变化
- **补足二级动作和微表演**：在不改设定的前提下，补充视线、呼吸、重心、步伐、手指、头发、衣摆、道具等细节运动，增强真实感
- **角色外观不重复堆砌**：角色身份、场景和道具辨识尽量放在 \`参考定义\` 中，分镜正文只保留与动作理解直接相关的必要信息
- **台词与嘴型严格匹配**：普通对白必须有开口说话和口型同步；内心OS/画外音VO必须明确嘴部不动，或角色不在画面内；无台词/少台词分镜优先按剧情补足普通话对白、内心OS或画外音
- **BGM 必须匹配剧情情绪**：每个提示词必须包含「BGM/背景音乐」段，根据当前轨道的情绪变化、冲突强度和台词语气描述配乐类型、节奏、音量和起伏；不得只写“合适的BGM”，不得使用歌词，BGM 音量必须低于对白和关键音效
- **无分镜图也可生成**：当 \`prompt\` 为空或 \`shouldGenerateImage=false\` 时，仍按 \`videoDesc\` 和关联资产完成视频提示词，不要因为缺少分镜图而中断
- **严格遵循输入**：不编造未出现的角色、道具、场景和剧情，不串镜，不改变时长含义，单分镜最低 1s

#### prompt 生成模板

> **注意**：\`参考定义\` 用于浓缩声明角色、场景、道具和必要的静态构图信息；分镜正文重点写运动、镜头、神态推进，不要照抄整段静态图提示词。

**单分镜模板：**
\`\`\`
画面风格和类型: {风格}, {色调}, {类型}

参考定义:
@图1: {角色/场景/道具名字}，{简短辨识描述}
@图2: {角色/场景/道具名字}，{简短辨识描述}
...

生成一个由以下 1 个分镜组成的视频:

场景:
分镜过渡: 无
BGM/背景音乐: {根据剧情情绪选择的配乐气质、节奏、音量与起伏；无歌词，低于对白，不遮盖环境音}

分镜1 {A}s: 时间：{...}，场景：{...}，镜头：{景别}，{角度}，{运镜}。主体关系：{谁在前景/中景/后景，谁为当前焦点}。动作节奏上，先{起始状态与动作}，随后{主要动作推进、运镜变化和神态变化}，最后{收束动作或情绪落点}。补充{视线、呼吸、手部、衣摆、头发、道具等二级动作}。{台词类型、台词内容、口型状态与音色描述（如有）}。{仅保留对动作或情绪有帮助的必要环境/光影/音效信息}。
\`\`\`

**多分镜模板：**
\`\`\`
画面风格和类型: {风格}, {色调}, {类型}

参考定义:
@图1: {角色/场景/道具名字}，{简短辨识描述}
@图2: {角色/场景/道具名字}，{简短辨识描述}
...

生成一个由以下 {N} 个分镜组成的视频:

场景:
分镜过渡: {整体过渡方式，说明焦点、景别或情绪如何在分镜间连续变化；所有分镜时长总和={总时长}s}
BGM/背景音乐: {根据全轨道剧情情绪设计配乐，从起幅到高潮/收束的强弱变化；无歌词，低于对白，不遮盖环境音}

分镜1 {A}s: ...
分镜2 {B}s: ...
...
\`\`\`

#### 音色生成规则（有台词时必填）

台词格式：\`{角色名字} 说：「{台词内容}」音色：{9维度描述}\`

9维度按顺序填写：
\`\`\`
{性别}，{年龄音色}，{音调}，{音色质感}，{声音厚度}，{发音方式}，{气息}，{语速}，{特殊质感}
\`\`\`

> 当素材未明确音色信息时，根据角色类型从以下参考表推断：

| 角色类型特征 | 默认音色 |
|------------|---------|
| 男性权威/霸气角色 | 男声，中年音色，音调低沉，音色浑厚有力，声音厚重，发音标准，气息极其沉稳，语速偏慢 |
| 女性温柔/甜美角色 | 女声，青年音色，音调中等偏高，音色质感明亮清脆，声音清亮柔和，发音方式干净，气息充沛平稳，语速适中，带温婉真诚感 |
| 男性年轻/普通角色 | 男声，青年音色，音调中等，音色干净，声音厚度适中，发音清晰，气息平稳，语速适中 |
| 女性活泼/外向角色 | 女声，青年音色，音调偏高，音色清脆活泼，声音轻盈，气息充沛，语速偏快，带笑意和感染力 |
| 反派/冷酷角色 | 男声，中年音色，音调低沉，音色质感干燥偏暗，声音带沙砾感，发音克制，气息平稳，语速极慢，有威胁感 |

#### 无台词/少台词分镜处理
- 无台词或台词少于每秒约 2 个中文字符时，按当前剧情、角色动作、情绪和冲突补充中文普通话对白、内心OS或画外音，使有效台词接近每秒 2-3 字
- 补充台词不得新增角色、道具、场景或剧情结果，不提前透露后续剧情；优先补角色反应、质问、解释、旁白或内心OS
- 只有剧情明确需要沉默时，才写 \`无台词，嘴部保持闭合\`

#### 台词类型格式

| 台词类型 | 格式 | 嘴型描述 |
|----------|------|----------|
| 普通对白 | \`{角色名字} 说：「{台词}」音色：{9维度}\` | 角色嘴部开合说话，口型同步 |
| 内心独白 | \`{角色名字} 内心OS：「{台词}」音色：{9维度}\` | 角色嘴部紧闭不动 |
| 画外音 | \`{角色名字} 画外音VO：「{台词}」音色：{9维度}\` | 角色嘴部紧闭不动，或角色不在画面内 |

#### 生成约束
1. **中文提示词**
2. **直接输出视频提示词**：禁止输出分析过程、推理步骤、模型匹配说明、分隔线等非提示词内容，第一行必须是 \`画面风格和类型:\`
3. **先参考定义，后写分镜**：必须先输出 \`参考定义\`，再输出分镜正文
4. **静态 prompt 只做辅助**：可吸收其中的景别、角度、构图、站位和已确定元素关系，但禁止整段照抄，不得让静态 prompt 覆盖 \`videoDesc\` 的动作与叙事
5. **每个分镜必须有动态递进**：至少写清主体动作、镜头运动、神态变化中的两项，优先用“先/随后/最后”组织成连续变化
6. **正文不重复堆砌外观**：不要在分镜正文中反复铺陈服饰、五官、材质等静态信息，除非它直接影响动作理解
7. **台词不可缺失**：已有台词必须完整输出台词内容和音色；普通对白、内心OS、画外音要用正确格式
8. **低台词密度需补足**：无台词或台词少于每秒约 2 字时，按剧情补足中文普通话对白、内心OS或画外音到每秒约 2-3 字；只有明确沉默时才标注嘴部不动
9. **BGM 必须输出**：必须单独写 \`BGM/背景音乐:\`，按剧情情绪写出配乐气质、节奏、强弱起伏和音量层级；不得使用歌词，不得遮盖中文对白、环境音或关键音效
10. **不编造未给定内容**：不新增角色、不新增道具、不新增场景，不跨分镜串内容
11. **时长单位与总时长**：直接使用秒，格式为 \`{N}s\`，单分镜最低 1s；所有分镜时长相加必须严格等于 XML duration 总和，且不超过 15s；不要把不足 15s 的轨道强行延展到 15s
12. **当关联资产顺序与类型混排时**，仍要依据资产真实类型组织参考定义和正文，不要按编号臆断角色/场景类型
13. **虚拟人像只影响接口参考图**：即使角色参考来自火山官方 \`asset://\` 虚拟人像，输出文本也按普通角色参考处理，只写 \`@图N: 角色名，简短辨识描述\` 和分镜动作
14. **衍生资产和分镜不替代虚拟人像**：角色衍生图、人物分镜图不得替代官方虚拟人像作为 Seedance 2.x 真人参考；相关服装/动作/构图只能写入提示词正文

#### Seedance 2.0 完整示例

输入：
\`\`\`
模型：seedance-2-0
资产信息[A001, role, 沈辞], [A002, role, 苏锦], [A003, scene, 城楼]
\`\`\`
\`\`\`xml
<storyboardItem videoDesc='（沈辞独立城楼远眺苍茫大地、城楼、沈辞/城楼、4s、全景、静止、负手而立衣袂随风飘扬、坚定决绝、黄昏冷调侧逆光、无台词、风声衣袂声、A001/A003）' prompt='全景，平视略仰，城楼之上，沈辞负手而立，衣袂飘扬，黄昏冷调侧逆光...' track='main' duration='4' associateAssetsIds="[&quot;A001&quot;,&quot;A003&quot;]" shouldGenerateImage="true"></storyboardItem>
<storyboardItem videoDesc='（苏锦登上城楼走向沈辞、城楼、苏锦/沈辞/城楼、4s、中景、跟踪、苏锦拾级而上走向沈辞、担忧、黄昏余晖渐暗、苏锦说：你又一个人在这里、脚步声风声、A001/A002/A003）' prompt='中景，跟踪，苏锦拾级而上走向城楼上的沈辞...' track='main' duration='4' associateAssetsIds="[&quot;A001&quot;,&quot;A002&quot;,&quot;A003&quot;]" shouldGenerateImage="true"></storyboardItem>
\`\`\`

输出：
\`\`\`
画面风格和类型: 真人写实, 电影风格, 冷调, 古风

参考定义:
@图1: 沈辞，青年男性角色参考
@图2: 苏锦，青年女性角色参考
@图3: 城楼，古代砖石城楼与台阶场景
@图4: 分镜构图参考，全景略仰，沈辞独立城楼
@图5: 分镜构图参考，中景跟踪，苏锦拾级而上

生成一个由以下 2 个分镜组成的视频:

场景:
分镜过渡: 从城楼上的独处凝望平滑过渡到人物接近后的对话，景别由全景转向中景，焦点从沈辞单人转为两人关系；整体时长严格控制为8秒。
BGM/背景音乐: 低音量古风弦乐与低频氛围铺底，前4秒压抑克制，苏锦靠近后加入轻微紧张拨弦，整体低于对白和风声，不使用歌词。

分镜1 4s: 时间：黄昏，场景：城楼，镜头：全景，平视略仰，起幅静止后轻微缓推。沈辞位于画面中前景偏右，为当前焦点。动作节奏上，先负手立定望向远处，随后视线轻微移动、下颌微收，衣袂与发梢被风持续带起，最后呼吸放缓但仍保持凝视。补充衣摆回摆、肩部细微起伏、风声层次和目光停顿。无台词，嘴部保持闭合。风声与衣袂摩擦声持续，冷调侧逆光压住人物情绪。
分镜2 4s: 时间：黄昏，场景：城楼，镜头：中景，平视，跟踪拍摄。苏锦从后侧台阶进入并逐步成为前景焦点，沈辞保持在前方中景。动作节奏上，先苏锦加快两步拾级而上，随后放慢脚步看向沈辞，眉眼收紧、手指轻收，最后开口说道：「你又一个人在这里。」音色：女声，青年音色，音调中等偏高，音色质感明亮清脆，声音清亮柔和，发音方式干净，气息充沛平稳，语速适中，带温婉真诚感，口型同步说话。补充脚步顿挫、衣摆轻晃、靠近前的停顿和视线停留。脚步声、风声与衣料轻响自然衔接。
\`\`\`

---

`;

export const grokImagineVideoPromptSection = `### Grok Imagine Video

#### 核心原则
- **只生成单段视频提示词**：Grok Imagine Video 不是 Seedance 2.0 的多分镜模板，不输出“生成一个由以下 N 个分镜组成的视频”、不输出“分镜1 7s”这类分镜列表。
- **按系统目标时长输出**：Grok Imagine Video 1.5 Preview 当前按 3-15 秒范围提交；最终提示词必须使用系统传入的目标时长，不要自行改成 6 秒、10 秒或其它固定秒数。
- **英文优先**：最终输出用自然英文视频提示词，中文角色名、台词内容可以保留原文；不要输出中文模板标题、分析过程或模型说明。
- **台词表演必须详细**：已有台词必须保留中文原文，并写清 speaker、dialogue/OS/VO 类型、声线/音色、当句情绪、语气、语速、气息停顿、音量重音和口型同步；不得省略、翻译或改写。无台词/少台词片段需按剧情补充中文普通话对白、OS 或 VO 到每秒约 2-3 字。
- **BGM must be included**：Every final prompt must include a concise "Background music/BGM" cue derived from plot mood, emotional turn, action intensity, and dialogue tone; keep it non-lyrical, low-volume, under Mandarin dialogue and key sound effects.
- **参考图使用方式**：Grok 支持在提示词中用 @图N 引用已上传图片；必须按当前输入顺序使用 @图1、@图2、@图3...，让角色、场景、道具和分镜构图分别绑定到对应参考图。
- **引用必须可追踪**：可以输出英文 "Reference mapping:" 行声明 @图N 对应的角色/场景/道具/分镜图；后续动作描述中继续使用这些 @图N，避免只写“provided reference images”导致模型混淆。
- **单段连续镜头**：将输入的一个或多个 storyboardItem 整合成一个连续 3-15 秒镜头，可按系统目标时长拆成 3-5 个 time beats，但不要拆成多个独立镜头。
- **动态优先**：每个 time beat 都要写动作推进、镜头运动、表情/姿态变化和情绪落点；不要只复述静态 prompt。
- **减少无效配置词**：避免 4K、best quality、ultra detailed 等堆砌；优先写主体、动作、镜头、光影、节奏、约束。
- **长度上限**：最终提示词必须控制在 3200 个英文字符以内；压缩 Reference mapping、Action timeline、Camera、Performance、Constraints，禁止复制长段 storyboard 原文。
- **视频稳定性约束**：必须加入 identity consistency、stable anatomy、natural motion、no extra characters、no subtitles、no text overlay、no logo/watermark 等负向约束。

#### 输出模板

\`\`\`
A {targetDuration}-second cinematic video clip. Use the provided reference image(s) to preserve exact character identity, outfit, scene design, and visual style.
Reference mapping: @图1 = {role/scene/prop/storyboard name and purpose}, @图2 = {...}.
Scene and mood: {one concise sentence based on videoDesc and art style}.
Action timeline: {0-Xs beat using @图N references}, {X-Ys beat using @图N references}, {Y-{targetDuration}s beat using @图N references}.
Camera and motion: {shot size, angle, camera path, focus change, natural secondary motion}.
Performance: {facial expression, gaze, posture, body language, mouth state; if dialogue exists, include speaker + exact Chinese line + dialogue type + voice texture + emotion + tone + speech rate + breath/pauses + volume/stress + lip-sync state}.
Background music/BGM: {non-lyrical music style, tempo, volume, and emotional rise/fall based on the story mood; keep under dialogue and key SFX}.
Lighting and atmosphere: {time of day, color temperature, light direction, environmental motion/sound cues as visual rhythm}.
Constraints: identity consistency, stable anatomy, natural motion, coherent hands and faces, no extra characters, no scene drift, no subtitles, no text overlay, no logo, no watermark.
\`\`\`

#### 时长组织
- 4-5 秒：优先分成起幅、推进、收束三段，动作必须直接有效。
- 6-8 秒：优先分成 0-30%、30-70%、70-100% 三段；多分镜输入时按原始顺序映射到这些节奏段。
- 9-10 秒：可分成 3-4 个 time beats，必须尽量覆盖全部台词与动作落点。
- 若输入分镜原始累计时长与系统提供的“Grok 目标时长”不一致，以系统目标时长为准，压缩或延展已有动作，不新增剧情结果。

#### 生成约束
1. 只输出最终英文提示词正文。
2. 第一行必须以系统指定的 \`A {targetDuration}-second cinematic video clip.\` 开始；targetDuration 必须来自系统输入，不得自行改写。
3. 必须使用 @图N 绑定参考图片；禁止使用未出现在输入中的 @图N，禁止把多个角色/场景都笼统写成 provided references。
4. 可以输出英文 \`Reference mapping:\`，但不要输出中文“参考定义/图片定义”、Markdown 标题、XML、解释说明。
5. 保留 videoDesc 中已存在的角色、场景、道具、台词和情绪，不新增未出现资产；无台词/少台词片段只允许补充符合当前剧情的中文普通话对白、OS 或 VO。
6. 有普通对白时，必须写 speaker、原中文台词、声线/音色、当句情绪、语气、语速、气息停顿、音量重音和 visible lip movement / lip-sync active；内心OS/VO 必须写 silent lips / mouth closed。
7. 无台词或台词少于每秒约 2 字时，先按剧情补足中文普通话对白、OS 或 VO 到每秒约 2-3 字；只有剧情明确沉默时才写 no dialogue, mouth remains closed。
8. 必须包含 \`Background music/BGM:\`，按剧情情绪写配乐类型、节奏、音量和起伏；不得使用歌词，不得盖过中文对白或关键音效。
9. 总长度不超过 3200 个英文字符。
`;

export function patchVideoPromptGenerationSeedance2Section(prompt: string): string;
export function patchVideoPromptGenerationSeedance2Section(prompt: null | undefined): null | undefined;
export function patchVideoPromptGenerationSeedance2Section(prompt: string | null | undefined) {
  if (prompt == null) return prompt;
  let next = prompt;
  if (next.includes("### 三、Seedance 2.0") && next.includes("### 四、Wan 2.6")) {
    next = next.replace(seedance2SectionPattern, seedance2PromptSection);
  }

  const section = `${videoPromptDocPresetStart}\n${videoPromptDocPresetSection.trim()}\n${videoPromptDocPresetEnd}`;
  const sectionPattern = new RegExp(`${videoPromptDocPresetStart}[\\s\\S]*?${videoPromptDocPresetEnd}`, "m");
  if (sectionPattern.test(next)) return next.replace(sectionPattern, section);
  if (next.includes("## 执行流程")) return next.replace("## 执行流程", `${section}\n\n---\n\n## 执行流程`);
  return `${next.trimEnd()}\n\n${section}\n`;
}
