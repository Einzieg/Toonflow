  ...
  const tables: TableSchema[] = [
    ...
    //提示词表
    {
      ...
      initData: async (knex) => {
        ...
        const videoPromptGeneration = await knex("o_prompt").where("type", "videoPromptGeneration").first();
        ...
        if (Object.keys(videoPromptPatch).length > 0) {
          await knex("o_prompt").where("type", "videoPromptGeneration").update(videoPromptPatch);
        }
      },
    },
    //小说原文表
