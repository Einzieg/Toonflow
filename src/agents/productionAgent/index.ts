...
async function createSubAgent(parentCtx: AgentContext) {
  ...
  //         "分镜面板：<storyboardItem videoDesc='视频描述' prompt='提示词内容' track='分组' duration='视频推荐时间' associateAssetsIds='[该分镜所需的资产ID列表]' shouldGenerateImage='true/false'></storyboardItem>",
  //         "```",
  //       ].join("\n");
  ...
  const run_sub_agent_storyboard_panel = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const addPrompt =
        "\n你必须使用如下XML格式写入工作区：\n```\n<storyboardItem videoDesc='视频描述' prompt='提示词内容' track='分组' duration='视频推荐时间' associateAssetsIds='[该分镜所需的资产ID列表]' shouldGenerateImage='true/false'></storyboardItem>\n```";
