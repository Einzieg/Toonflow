    ...
    <t-card bordered :style="{ height: '100%' }">
      ...
      <div class="itemBox">
        <div
          ...
          <t-checkbox
            class="trackCheck"
            :checked="isTrackChecked(track.id)"
            @click.stop
          ...
          <div class="thumbGroup" v-if="track.selectVideoId && getSelectedVideoSrc(track)">
            <img
              v-if="getSelectedVideoCover(track)"
              class="thumb selectedVideoThumb"
              :src="getSelectedVideoCover(track)!"
              draggable="false" />
...
const generatingMap = defineModel<Record<number, boolean>>("generatingMap", {
  default: () => {},
});
...
function getSelectedVideoCover(track: TrackItem): string | null {
  const src = getSelectedVideoSrc(track);
  return src ? videoCoverMap.value[src] || null : null;
}

function isTrackChecked(trackId: number | undefined) {
  return trackId != null && checkedTrackIdSet.value.has(trackId);
}

function getTrackImageThumb(src?: string) {
...
watch(
  () => trackList.value.map((track) => getSelectedVideoSrc(track) ?? ""),
  (sources) => {
    sources.forEach((src) => {
