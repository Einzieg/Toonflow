    ...
    <div class="data" @click="selectedFn">
      ...
      <div class="image">
        ...
        <div v-else class="imageWrapper">
          <t-image class="image" :src="currentGeneratedImage" fit="contain" :class="['nodeImage', { selected }]">
            <template #overlayContent>
              <div class="imageToolsWrap">
                <ImageTools :src="currentGeneratedImage" position="br" />
              </div>
            </template>
          </t-image>
        </div>
...
watch(
  ...
  (newUrl) => {
    currentGeneratedImage.value = newUrl || "";
  },
  { immediate: true },
);

function setGeneratedImage(imageUrl: string) {
  currentGeneratedImage.value = imageUrl;
...
function handleKeep() {
  if (!currentGeneratedImage.value) return window.$message.error($t("workbench.production.editImage.generateFirst"));
  emit("keep", currentGeneratedImage.value);
}
onMounted(() => {
  const nextData: Partial<GeneratedNodeData> = {};
  if (!props.data.model) nextData.model = project.value?.imageModel ?? "";
  if (!props.data.quality) nextData.quality = project.value?.imageQuality ?? "";
  if (!props.data.ratio) nextData.ratio = project.value?.videoRatio ?? "16:9";
