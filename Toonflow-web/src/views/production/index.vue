<template>
  <VueFlow
    ...
    <template #node-storyboard="props">
      <storyboard v-memo="[storyboardNodeMemo]" :id="props.id" v-model="flowData.storyboard" :assetsData="flowData.assets" :handleIds="props.data.handleIds" />
    </template>
    ...
    <div class="floatingWindow">
      ...
      <transition name="slide">
        <rightChatBox v-if="episodesId && openShowVisible" v-memo="[title, openShowVisible]" :title="title" @close="openShowVisible = false" />
      </transition>
    </div>
...
const scriptNodeMemo = computed(() => flowData.value.script);
const scriptPlanNodeMemo = computed(() => flowData.value.scriptPlan);
const storyboardTableNodeMemo = computed(() => flowData.value.storyboardTable);
const assetsNodeMemo = computed(() =>
  flowData.value.assets
    .map((item) => {
      const deriveKey = item.derive
        .map((derive) => `${derive.id}:${derive.src}:${derive.state}:${derive.errorReason ?? ""}`)
        .join(",");
      return `${item.id}:${item.src}:${item.state}:${item.errorReason ?? ""}:${deriveKey}`;
    })
    .join("|"),
);
const storyboardNodeMemo = computed(() =>
  `${flowData.value.storyboard
    .map((item) =>
      [
        ...
        item.videoDesc,
        item.duration,
        item.shouldGenerateImage,
        item.associateAssetsIds?.join(",") ?? "",
      ].join(":"),
    )
    .join("|")}__${assetsNodeMemo.value}`,
);
const workbenchNodeMemo = computed(() =>
  flowData.value.workbench.videoList.map((item) => `${item.id}:${item.storyboardId}:${item.trackId}:${item.duration}:${item.prompt}`).join("|"),
);
...
.flowMain {
  ...
  :deep(.vue-flow__node .t-card) {
    box-shadow: none !important;
    background-image: none !important;
    border: 1px solid var(--td-border-level-1-color, #e7e7e7);
    contain: layout style;
  }
  :deep(.vue-flow__node .t-card__body) {
    transition: none !important;
  }
  :deep(.vue-flow__node .t-card:hover),
  :deep(.vue-flow__node .t-card:focus-within) {
    box-shadow: none !important;
  }
  &.space-dragging {
...
.flowMain.is-interacting {
  ...
  :deep(.assetImageWrap),
  :deep(.frameImage),
  :deep(.videoPlaceholder) {
    background-image: none !important;
    background-color: var(--td-bg-color-container-hover, #f5f5f5) !important;
  }
  // 禁用 hover 效果，减少样式重算
  :deep(.imageToolsWrap),
  :deep(.addBetween),
